...
async function obtenerDatosTickets() {
    try {
        ...
        if (response.ok) {
            ...
            mostrarDatosEnInterfazTickets(data);
        } else {
            console.error("Error al obtener los datos de tickets");
...
function mostrarDatosEnInterfazTickets(data) {
    ...
    if (data.dependencias) agregarDatosATabla('dependenciaTable', data.dependencias, 'nombre', 'estado', 'iddependencia');
    if (data.soporte) agregarDatosATabla('soporteTable', data.soporte, 'nombre', 'estado', 'idsoporte');
    if (data.temas) agregarDatosATabla('temasTable', data.temas, 'nombre', 'estado', 'idtema');
}

// Evento para cargar datos solo cuando se abra un modal en la vista de tickets
$(document).on('shown.bs.modal', function (event) {
    const modalId = $(event.target).attr('id');
    const modalesTickets = ['nuevoSoporteModal', 'nuevoTemasModal', 'nuevaDependenciaModal'];

    if (modalesTickets.includes(modalId)) {
        obtenerDatosTickets();
    }
});
...
document.querySelectorAll("[id^=registrar]").forEach((button) => {
    button.replaceWith(button.cloneNode(true)); // Reemplaza el botón por una copia sin eventos previos
});
